...
public class DsgcEnvParser {
    ...
    public static void main(String[] args) throws IOException, CsvValidationException {
        ...
        final String fileName = "Rollout_Stage_2.csv";
        final Map<String, DsgcEnv> dsgcEnv = LinkedHashMap.newLinkedHashMap(12000);

        ReadUtils.readDsgcEnvs(dsgcEnv, fileName);

        List<String> dsgcEnvs = dsgcEnv.values().stream().filter(e -> e.candidates() <=10000000000000L && e.envType() == EnvType.PROD).map(DsgcEnv::toString).toList();
